...
class YoLov5TRT(object):
    ...
    def infer(self, raw_image_generator, plot=True, conf_thresh=0.5, iou_thresh=0.4):
        ...
        for i in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.post_process(output[i * 6001:(i + 1) * 6001],
                                                                            batch_origin_h[i],
                                                                            batch_origin_w[i],
                                                                            conf_thresh,
                                                                            iou_thresh)
            ...
            if not plot:
                batch_result[i] = dict(bboxes=result_boxes,
                                       scores=result_scores,
                                       class_ids=[int(x) for x in result_classid])
            else:
